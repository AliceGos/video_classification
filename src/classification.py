...
train_formodel  = train  # can be switched to train_all if explore results on surf+longboard
test_formodel   = test   # can be switched to test_all if explore results on surf+longboard
y_test_formodel = y_test # can be switched to y_test_all if explore results on surf+longboard
kfold = 4
...
for k in range(kfold):
    ...
    # Normalization
    cols_to_scale = train_set.columns[:6]
    ...
    test_formodel[cols_to_scale] = scaler.transform(test_formodel[cols_to_scale])
    ...
    crossentropy_weights=torch.tensor([0.4,0.3])
    criterion = torch.nn.CrossEntropyLoss(weight=crossentropy_weights)
    nepochs = 150
